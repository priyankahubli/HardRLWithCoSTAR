...
def train_featurizer(featurizer_type, videos_path, framerate, initial_width, initial_height,
                     desired_width, desired_height, epochs, batch_size, learning_rate, costar,
    ...
    """
    # Prepare dataset
    dataset = generate_dataset(videos_path, framerate, initial_width, initial_height,costar)
    ...
    featurizer = featurizer_class(initial_width, initial_height, desired_width, desired_height,costar, feature_vector_size=1024, learning_rate=learning_rate, experiment_name='default')
...
def generate_dataset(videos_path, framerate, width, height,costar):
    ...
    if (costar):
        for video_name in os.listdir('videos/{}'.format(videos_path)):
            print('Video: {}'.format(video_name))
            path = 'videos/{}/{}'.format(videos_path, video_name)
            with h5py.File(os.path.expanduser(path), 'r') as data:
                    images = ConvertImageListToNumpy(data['image'])
            ...
            for video_name in os.listdir('videos/{}/{}'.format(videos_path, playlist)):
                ...
                while video.isOpened():
                    ...
                    if success:
                        ...
                        break
            dataset[video_index] = np.reshape(dataset[video_index], (-1, width, height, channels))
            video_index += 1
    return dataset
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--costar', help='True if training on costar dataset', type=bool, default=True)

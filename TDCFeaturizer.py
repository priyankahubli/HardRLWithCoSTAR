...
class TDCFeaturizer(BaseFeaturizer):
    ...
    def __init__(self, initial_width, initial_height, desired_width, desired_height, costar,feature_vector_size=1024, learning_rate=0.0001, experiment_name='default'):
        print("Starting featurizer initialization")
        self.sess = tf.Session()
        self.graph = TDCFeaturizer._generate_featurizer(initial_width, initial_height, desired_width, desired_height, feature_vector_size, learning_rate,costar)
    ...
    def _generate_featurizer(initial_width, initial_height, desired_width, desired_height, feature_vector_size, learning_rate,costar):
        ...
        """
        channels = 3 if costar else 1
        is_training = tf.placeholder(dtype=tf.bool, shape=(), name="is_training")
        stacked_state = tf.placeholder(dtype=tf.float32, shape=(None, 2, initial_width, initial_height, channels), name="stacked_state")
        labels = tf.placeholder(dtype=tf.float32, shape=(None, 6), name="labels") # There are 6 possible labels

        state = tf.reshape(stacked_state, (-1, initial_width, initial_height, channels))
        state = tf.random_crop(state , (tf.shape(state)[0], desired_width, desired_height, channels))
        ...
        with tf.variable_scope('Encoder'):
            ...
            #print('feature vector stack shape: ',feature_vector_stack.shape)
    ...
    def _generate_training_data(videos, number_of_samples):
        ...
        for i in range(number_of_samples):
            ...
            #print('error: ', videos[video_index].shape,  possible_frames_end )
